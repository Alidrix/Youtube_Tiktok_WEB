...
case "$BACKUP_FILE" in *.sql|*.sql.gz) ;; *) fail "Unsupported backup format" ;; esac
if [ -f "${BACKUP_FILE}.sha256" ]; then info "Verifying checksum"; sha256sum -c "${BACKUP_FILE}.sha256"; else info "No checksum file found, skipping"; fi
info "Starting temporary PostgreSQL container: $CONTAINER_NAME"
docker run -d --name "$CONTAINER_NAME" -e POSTGRES_USER="$POSTGRES_USER" -e POSTGRES_PASSWORD="$POSTGRES_PASSWORD" -e POSTGRES_DB="$POSTGRES_DB" "$POSTGRES_IMAGE" >/dev/null
for _ in $(seq 1 40); do
  if docker exec "$CONTAINER_NAME" pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB" >/dev/null 2>&1; then break; fi
  sleep 1
done
...
if [[ "$BACKUP_FILE" == *.sql.gz ]]; then gzip -dc "$BACKUP_FILE" | docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" "$POSTGRES_DB" >/dev/null; else docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" "$POSTGRES_DB" < "$BACKUP_FILE" >/dev/null; fi
