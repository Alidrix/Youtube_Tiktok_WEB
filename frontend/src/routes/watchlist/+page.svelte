<script lang="ts">
  ...
  let items: any[] = []; let name=''; let selectedPlatforms=['youtube']; let selectedRegions=['FR']; let selectedCategories:string[]=[]; let error=''; let loading=false;
  ...
  async function create(){await createWatchlist({name,keywords:[],categories:selectedCategories,platforms:selectedPlatforms,regions:selectedRegions}); name=''; await load();}
